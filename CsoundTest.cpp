...
void
CsoundTest::log(string_view message) {
...
CsoundTest::compileCsdFile(){
    ...
    if (_savedCsdPath.length() > 0) {
        ...
        if (_csCompileResult == 0) { // compiled OK...
            ...
            // display the p-fields
//            updateParameter(6, _csound->GetStringChannel("Pfields", _displayedParams));
//            log("Pfields = " + to_string(_csound->GetStringChannel("Pfields")));
            string temp = "some parameter info:\np4 = pitch\n";
//            log(temp);
            updateParameter(6, temp);
